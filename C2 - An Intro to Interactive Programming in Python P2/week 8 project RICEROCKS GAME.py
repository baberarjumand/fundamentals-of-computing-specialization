# http://www.codeskulptor.org/#user47_p323rvMzUr_0.py
...
#        a_missile = Sprite(missile_pos, missile_vel, self.angle, 0, missile_image, missile_info, missile_sound)
        missile_group.add(Sprite(missile_pos, \
                                 missile_vel, \
                                 self.angle, \
                                 0, \
                                 missile_image, \
...
#    a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    global a_rock, rock_group, score, started
    
    if started:        
        rock_position = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_velocity = [random.random() * .6 - .3, random.random() * .6 - .3]
        rock_angular_velocity = random.random() * .2 - .1
        if (len(rock_group) < 12) and (dist(rock_position, my_ship.pos) > my_ship.radius * 3):            
            rock_group.add(Sprite(rock_position, \
                                  rock_velocity, \
                                  0, \
                                  rock_angular_velocity, \
                                  asteroid_image, \
                                  asteroid_info))
...
def group_collide(group_sprite, other_object):
    collidedFlag = False
