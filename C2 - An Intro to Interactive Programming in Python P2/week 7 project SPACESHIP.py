# http://www.codeskulptor.org/#user47_xFvzftJkf1_0.py
...
class Ship:
    ...
    def shoot(self):
        ...
        current_position = [ self.pos[0] + ( fwd_vector[0] * self.radius), \
                             self.pos[1] + ( fwd_vector[1] * self.radius) ]
        a_missile.pos = current_position
        a_missile.vel = [ self.vel[0] + (fwd_vector[0] * 6), \
                          self.vel[1] + (fwd_vector[1] * 6) ]
        missile_sound.play()
    ...
    def update(self):
        ...
        if self.thrust:        
            self.vel[0] += angle_to_vector(self.angle)[0] * 0.125 
            self.vel[1] += angle_to_vector(self.angle)[1] * 0.125
...
def keydown(key):
    global my_ship, ship_thrust_sound
...
def draw(canvas):
    global time, my_ship, a_rock, a_missile, lives, score, WIDTH, HEIGHT
    ...
    canvas.draw_text(str(lives), \
                     ((WIDTH * 0.25) + 85, HEIGHT * 0.10), \
                     32, \
                     'white')
    canvas.draw_text('Score: ', \
                     (WIDTH * 0.75, HEIGHT * 0.10), \
                     32, \
                     'white')
    canvas.draw_text(str(score), \
                     ((WIDTH * 0.75) + 85, HEIGHT * 0.10), \
                     32, \
                     'white')
...
def rock_spawner():
    ...
    a_rock.vel[0] = random.randrange(-10, 10) * 0.10
    a_rock.vel[1] = random.randrange(-10, 10) * 0.10
    
    a_rock.angle_vel = random.choice([-1, 1]) * 0.10
